...
func (h *LeaderboardHandler) Leaderboard(ctx context.Context, req *connect.Request[v1.LeaderboardRequest]) (*connect.Response[v1.LeaderboardResponse], error) {
	handlerFunc := func(ctx context.Context, req *connect.Request[v1.LeaderboardRequest]) (*connect.Response[v1.LeaderboardResponse], error) {
...
func (h *LeaderboardHandler) Subleaderboard(ctx context.Context, req *connect.Request[v1.SubleaderboardRequest]) (*connect.Response[v1.SubleaderboardResponse], error) {
	handlerFunc := func(ctx context.Context, req *connect.Request[v1.SubleaderboardRequest]) (*connect.Response[v1.SubleaderboardResponse], error) {
		userID, ok := interceptor.GetUserID(ctx)
		if !ok {
			return nil, connect.NewError(connect.CodeUnauthenticated, nil)
		}

		leaderboard, err := h.svc.Subleaderboard(ctx, userID, req.Msg.SubcontestSlug, req.Msg.Limit, req.Msg.Offset)
		if err != nil {
			if err.Error() == "permission denied: no access to subcontest" {
